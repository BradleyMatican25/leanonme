...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      id = "tabs",
      menuItem("🎯 Key Results", tabName = "key_results", icon = icon("chart-line")),
    ...
    tabItems(
      ...
      tabItem(
        ...
        conditionalPanel(
          ...
          # Additional QoI Summary Table
          fluidRow(
            box(
              width = 12, status = "warning", solidHeader = TRUE,
              title = "📊 Complete QoI Summary with Confidence Intervals",
              div(style = "overflow-x: auto;",
                  tableOutput("qoi_summary_table")
              ),
              div(class = "help-text",
                  "All confidence intervals computed via bootstrapping simulation (10,000 iterations). ",
                  "Point estimates represent mean values from 6-month dataset (Jan-Jun 2024)."
              )
            )
          ),
          ...
          # Value Boxes Row
          fluidRow(
            valueBoxOutput("vb_holding_period", width = 3),
            valueBoxOutput("vb_turnover", width = 3),
            valueBoxOutput("vb_sales_growth", width = 3),
            valueBoxOutput("vb_capital_released", width = 3)
          ),
              ...
              fluidRow(
                column(4,
                       div(class = "info-metric",
                           div(class = "info-metric-label", "Baseline Annual Cost"),
                           div(class = "info-metric-value", style = "color: #dc3545;", "₱17,478"),
                           div(class = "info-metric-label", "IT Rate: 2.62 | Holding: 139 days")
                       )
                ),
                column(4,
                       div(class = "info-metric",
                           div(class = "info-metric-label", "Target Annual Cost"),
                           div(class = "info-metric-value", style = "color: #28a745;", "₱11,448"),
                           div(class = "info-metric-label", "IT Rate: 4.00 | Holding: 91 days")
                       )
                ),
                column(4,
                       div(class = "info-metric",
                           div(class = "info-metric-label", "Annual Savings (5 Stores)"),
                           div(class = "info-metric-value", style = "color: #007bff;", "₱6,030"),
                           uiOutput("financial_ci_display_box")
                       )
                )
              ),
              div(class = "key-insight",
      ...
      tabItem(
        ...
        conditionalPanel(
          ...
          fluidRow(
            box(
              width = 12, status = "primary", solidHeader = TRUE,
              title = "📊 Financial Breakdown & ROI Analysis",
              div(style = "padding: 20px;",
                  h4("Investment Return Summary"),
                  fluidRow(
                    column(4,
                           div(style = "text-align: center; padding: 20px; background: #e8f5e9; border-radius: 8px;",
                               div(style = "font-size: 0.9em; color: #2e7d32;", "Implementation Cost"),
                               div(style = "font-size: 2em; font-weight: bold; color: #1b5e20; margin: 10px 0;", "~₱0"),
                               div(style = "font-size: 0.8em; color: #388e3c;", "Process change only")
                           )
                    ),
                    column(4,
                           div(style = "text-align: center; padding: 20px; background: #e3f2fd; border-radius: 8px;",
                               div(style = "font-size: 0.9em; color: #1565c0;", "Payback Period"),
                               div(style = "font-size: 2em; font-weight: bold; color: #0d47a1; margin: 10px 0;", "Immediate"),
                               div(style = "font-size: 0.8em; color: #1976d2;", "No capital investment")
                           )
                    ),
                    column(4,
                           div(style = "text-align: center; padding: 20px; background: #f3e5f5; border-radius: 8px;",
                               div(style = "font-size: 0.9em; color: #6a1b9a;", "ROI (1 Year)"),
                               div(style = "font-size: 2em; font-weight: bold; color: #4a148c; margin: 10px 0;", "∞"),
                               div(style = "font-size: 0.8em; color: #7b1fa2;", "Zero-cost improvement")
                           )
                    )
                  ),
                  div(class = "key-insight", style = "margin-top: 20px;",
                      strong("💡 Key Insight: "),
                      "This Six Sigma project demonstrates the power of process improvement. By implementing ",
                      "WMA forecasting and ITO framework (no technology investment required), Papemelroti can ",
                      "achieve ₱25,326 in annual savings across all stores while releasing ₱126,626 in working ",
        ...
        fluidRow(
          box(
            ...
            fluidRow(
              column(6,
                     h4("🎓 Academic Context"),
                     p(strong("Course:"), "SYSEN 5300 - Six Sigma Methods for Process Improvement"),
                     p(strong("Institution:"), "Cornell University - Johnson Graduate School of Management"),
                     p(strong("Program:"), "MBA/MILR Dual Degree (Operations & HR Focus)"),
                     p(strong("Semester:"), "Fall 2024"),
                     
                     h4(style = "margin-top: 25px;", "👥 Project Team"),
                     tags$ul(
                       tags$li(strong("Bradley Matican"), "- Project Leader"),
                       tags$li(strong("Chris Lasa"), "- Data Specialist & Dashboard Development"),
                       tags$li(strong("Deepro Bandyopadhyay"), "- Statistical Analysis"),
                       tags$li(strong("Sreekar Mukkamala"), "- Process Optimization Analysis")
                     ),
